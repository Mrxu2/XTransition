...
@interface WKCollectionViewLayout()
{
//    NSMutableDictionary *_attributeDict;
//    NSInteger _cellCount;
//    NSMutableArray *_colArr;
//    UIEdgeInsets _edge;
    CGFloat _totoalHieght;
}
@end
@implementation WKCollectionViewLayout
...
- (void)layoutItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [_attributeDict setObject:NSStringFromCGRect(frame) forKey:indexPath];
...
//            NSSLog(@"%ld-------%ldld",(long)indexPath.row,(long)indexPath.section);
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    for (NSIndexPath *indexPath in _attributeDict)
    {
        CGRect cellRect = CGRectFromString(_attributeDict[indexPath]);
        if (CGRectIntersectsRect(cellRect, rect)) {
//            NSIndexPath *indexPath = _attributeDict[rectStr];
...
-(UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    attributes.frame = CGRectFromString(_attributeDict[indexPath]);
    return attributes;
...
//    NSSLog(@"%@+_+-=-=-=",NSStringFromCGSize(size));
    return size;
}
